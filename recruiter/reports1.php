...
$avgSalaryQuery = $pdo->prepare("
    SELECT job_applications.work_experience, AVG(offer_details.salary) AS avg_salary
    FROM job_applications
    JOIN offer_details ON job_applications.application_id = offer_details.application_id
    GROUP BY job_applications.work_experience
    ORDER BY job_applications.work_experience ASC
");
...
echo "<h3>Job-Specific Metrics</h3>";
echo "<table border='1' cellpadding='5' cellspacing='0'>";
echo "<tr>
        <th>Job Title</th>
        <th>Total Closed Job Posts</th>
        <th>Avg Time to Fill (Days)</th>
        <th>Avg Total Duration</th>
        <th>Total Applicants</th>
        <th>Total Hired</th>
        <th>Avg Dropout Rate</th>
      </tr>";

foreach ($jobPostMetrics as $job) {
    echo "<tr>
            <td>{$job['job_post_title']}</td>
            <td>{$job['total_closed_job_posts']}</td>
            <td>{$job['avg_time_to_fill']}</td>
            <td>{$job['avg_total_duration']}</td>
            <td>{$job['total_applicants_for_job_post']}</td>
            <td>{$job['total_hired_for_job_post']}</td>
            <td>{$job['avg_dropout_rate_for_job_post']}%</td>
          </tr>";
}

echo "</table>";

echo "<h3>Drop-Off Rates Per Job Post</h3>";
echo "<table border='1' cellpadding='5' cellspacing='0'>";
echo "<tr>
        <th>Job Title</th>
        <th>Total Applicants</th>
        <th>Drop-Off After Application</th>
        <th>Drop-Off After Screening</th>
        <th>Drop-Off After Interview</th>
        <th>Drop-Off After Offer</th>
        <th>Rejected</th>
        <th>Withdrawn</th>
      </tr>";

foreach ($dropoffPerJobPost as $job) {
    echo "<tr>
            <td>{$job['job_post_title']}</td>
            <td>{$job['total_applicants_for_job_post']}</td>
            <td>{$job['drop_off_after_application_for_job_post']}</td>
            <td>{$job['drop_off_after_screening_for_job_post']}</td>
            <td>{$job['drop_off_after_interview_for_job_post']}</td>
            <td>{$job['drop_off_after_offer_for_job_post']}</td>
            <td>{$job['total_rejected_for_job_post']}</td>
            <td>{$job['total_withdrawn_for_job_post']}</td>
          </tr>";
}
...
echo "<h3>Conversion Rates Per Job Post</h3>";
echo "<table border='1' cellpadding='5' cellspacing='0'>";
echo "<tr>
        <th>Job Title</th>
        <th>Screened Conversion Rate</th>
        <th>Interviewed Conversion Rate</th>
        <th>Offer Conversion Rate</th>
        <th>Hire Conversion Rate</th>
      </tr>";

foreach ($conversionRatesPerJobPost as $job) {
    echo "<tr>
            <td>{$job['job_post_title']}</td>
            <td>" . round($job['screened_conversion_rate_for_job_post'], 2) . "%</td>
            <td>" . round($job['interviewed_conversion_rate_for_job_post'], 2) . "%</td>
            <td>" . round($job['offer_conversion_rate_for_job_post'], 2) . "%</td>
            <td>" . round($job['hire_conversion_rate_for_job_post'], 2) . "%</td>
          </tr>";
}

echo "</table>";

echo "<h3>Interviews Per Hire Per Job Post</h3>";
echo "<table border='1' cellpadding='5' cellspacing='0'>";
echo "<tr>
        <th>Job Title</th>
        <th>Avg Interviews Per Hire</th>
      </tr>";

foreach ($interviewsPerHirePerJobPost as $job) {
    echo "<tr>
            <td>{$job['job_post_title']}</td>
            <td>" . round($job['avg_interviews_per_hire_for_job_post'], 2) . "</td>
          </tr>";
}
...
<body>
    ...
    <h2 id="totalApplicantsHeader">Total Applicants: 0</h2>
